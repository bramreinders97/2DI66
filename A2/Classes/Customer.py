...
class Customer:
    def __init__(self, arrival_time, group_index, card_only):
        ...
        self.departure_time = -1            # Departure-time of the customer. -1 if still in the system.

        self.payment_method = self.get_payment_method(card_only)    # can either be "cash" or "card".
        self.get_food_time = self.get_food_time()   # time the customer needs to find the products he/she likes.

        self.group_index = group_index  # The index of the group the customer belongs to.
        self.queue = None               # The queue to which the customer belongs to.
